...
function New-ProofImage {
    ...
    $graphics.DrawString("INTEGRATION EVIDENCE LEDGER", $eyebrowFont, $accentBrush, 92, 92)
...
New-ProofImage -Path (Join-Path $screenshots "01-overview-proof.png") `
    -Title "Integration handoff checkpoint posture" `
    -Subtitle "Operator-readable proof for contracts, delivery attestations, replay evidence, and escalation routing." `
    -Bullets @(
        "Surface the same packets connector owners, release leads, and auditors argue over.",
        "Keep handoff debt visible before it becomes a production or audit problem.",
        "Show recruiter-readable integration-governance depth."
    )

New-ProofImage -Path (Join-Path $screenshots "02-handoff-lane-proof.png") `
    -Title "Lane ownership and evidence routing" `
    -Subtitle "Each lane maps to a real handoff-governance responsibility instead of generic middleware copy." `
    -Bullets @(
        "Contract, delivery, replay, and fallback lanes stay separated and accountable.",
        "Owner mapping keeps governance, delivery, and escalation teams explicit.",
        "Findings roll up into checkpoint-safe next actions."
    )

New-ProofImage -Path (Join-Path $screenshots "03-review-posture-proof.png") `
    -Title "Review posture before checkpoint" `
    -Subtitle "Decision packets show which integrations can move forward and which should stop." `
    -Bullets @(
        "Completeness score, blocker, and checkpoint window stay in one view.",
        "High-severity packet debt is visible before critical handoffs go live.",
